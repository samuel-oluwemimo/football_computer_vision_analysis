...
class Trackers():
    ...
    def get_obj_tracks(self, frames, read_from_stub=False, stub_path=None):
        ...
        for idx, detection in enumerate(detections):
            ...
            for obj_ind, class_ind in enumerate(detection_supervision.class_id):
                if class_names[class_ind] == 'goalkeeper':
                    detection_supervision.class_id[obj_ind] = class_inv['player']

            detection_wth_trackers = self.tracker.update_with_detections(detection_supervision)

            tracks['players'].append({})
            tracks['referees'].append({})
            tracks['ball'].append({})

            for frame_detection in detection_wth_trackers:
    ...
    def draw_annotations(self,video_frames, tracks,team_ball_control):
        ...
        for frame_num, frame in enumerate(video_frames):
            ...
            for _, referee in referee_dict.items():
             ...
             # Draw Ball and its Trace
            for track_id, ball in ball_dict.items():
                frame = self.draw_traingle(frame, ball["bbox"], (255, 255, 0))  # Yellow for ball
